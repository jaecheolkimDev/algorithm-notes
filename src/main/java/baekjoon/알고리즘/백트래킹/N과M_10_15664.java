...
public class N과M_10_15664 {
	...
	public static int[] numbers; // 입력받은 자연수 배열
	public static StringBuffer sb = new StringBuffer();
	public static int[] selected; // 조건에 맞게 선택된 수
	public static boolean[] used; // 중복제거 위해 사용여부 파악 변수
	...
	public static void input() throws IOException {
		...
		numbers = new int[n + 1];	// 입력받은 자연수 배열
		for (int i = 1; i <= n; i++) {
	...
	public static void pro() {
		...
		used = new boolean[n + 1]; // 중복제거 위해 사용여부 파악 변수
	...
	public static void recFunc(int k) {
		...
		if (k == m + 1) {
			...
			int before = 0;	// 중복제거에 사용 : k번째에 사용한 수
			for (int i = 1; i <= n; i++) {
				if (used[i]) continue; // 중복제거 위해 사용여부 파악 변수
				if (before == numbers[i]) continue;	// k번째에 사용한 수 와 현재 사용할 수 가 똑같으면 제외.
				if (numbers[i] >= selected[k - 1]) { // 비내림차순 : 이전에 나온 수와 같거나 그보다 크다.
					// k 번째에 cand 가 올 수 있으면
					selected[k] = numbers[i];
					before = numbers[i];	// 중복제거에 사용 : k번째에 사용한 수
					used[i] = true;
