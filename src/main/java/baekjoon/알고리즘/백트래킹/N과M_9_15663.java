...
public class N과M_9_15663 {
	...
	public static boolean[] used; // 하나밖에 없는 수의 중복사용 방지.
	...
	public static void recFunc(int k) {
		...
		if (k == m + 1) {
			...
			int before = 0;	// 중복수열 제외
			for (int i = 1; i <= n; i++) {
				if (used[i]) continue; // 하나밖에 없는 수의 중복사용 방지.
				if (before == numbers[i]) continue;	// 중복수열 제외
				// k 번째에 cand 가 올 수 있으면
				selected[k] = numbers[i];
				before = numbers[i];	// 중복수열 제외
				used[i] = true;
