...
public class Fly_me_to_the_Alpha_Centauri_1011 {
	...
	public static void main(String[] args) throws Exception {
		...
		for(int i=0; i<t; i++) {
			...
			long x = Integer.parseInt(d[0]);	// 시작점
			long y = Integer.parseInt(d[1]);	// 도착점
			long tot = y-x;	// 이동해야 하는 거리
			
			// 이동한 횟수
			long ans = 0;
			
			long sqrt = (long)Math.sqrt(tot);	// 2
			long sqrt2 = sqrt*sqrt;	// 4
			if(tot == sqrt2) {
				ans = sqrt + sqrt - 1;
			} else if(tot > sqrt2) {
				ans = tot > sqrt2+sqrt ? sqrt + sqrt + 1 : sqrt + sqrt; 
			}
