...
public class ACM호텔_10250 {
	...
	public static void main(String[] args) throws Exception {
		...
		for(int i=0; i<t; i++) {
			s = br.readLine().split(" ");
			h = Integer.parseInt(s[0]);	// 총 몇 층
			w = Integer.parseInt(s[1]);	// 총 호수
			n = Integer.parseInt(s[2]);	// n번째 손님
			ans1 = String.valueOf(n>=h && n%h == 0 ? h : n%h);		// 층
			ans2 = String.valueOf(n>=h && n%h == 0 ? n/h : n/h + 1);	// 호수
			if(ans2.length() == 1) {
				sb.append(ans1 + "0" + ans2);		
			} else {
				sb.append(ans1 + ans2);		
			}
			sb.append("\n");
